...
tError infection_init(tInfection* object, tInfectiousAgent* infectiousAgent, tCountry* country, tDate* date){
    ...
    if(object->country == NULL || object->infectiousAgent == NULL || object->date == NULL) {
		// Some of the fields have a NULL value, what means that we found some problem allocating the memory
		return ERR_MEMORY_ERROR;
    }

    // Once the memory is allocated, copy the data. We use specific functions defined for copy those structures.
    ...
    // Initialize integer fields to 0
	object->totalCases = 0;
...
void infection_free(tInfection* object){
    // PR2_EX2 => modify this function
		
    // Verify pre conditions
    assert(object != NULL);

    // Use specific functions for free structures
...
void infection_update(tInfection* infection, int cases, int deaths, int criticalCases, int recovered){
    // Verify pre conditions
    if (infection == NULL)
        return; // Assert not used to avoid error when some functions are not implemented
...
void infection_update_recursive(tInfection* infection){
    // PR2_EX3
	infection_update(infection,country_totalCases(infection->country),
								country_totalDeaths(infection->country),
...
tInfection* infectionTable_find(tInfectionTable* table, const char* infectiousAgentName, tCountry* country){
    ...
    //assert(country != NULL); // Assert not used to avoid error when some functions are not implemented
