...
bool run_pr1_ex2(tTestSection* test_section) {
    bool passed = true, failed = false;
    tInfectiousAgent influenzaA1 = {0};
	tInfectiousAgent influenzaB = {0};
	tInfectiousAgent infectiousAgent_copy = {0};
    tDate date1, date2;
    ...
    infectiousAgent_init(&infectiousAgent_copy, "Influenza Yamagata", 0.15, "Air", &date2, "London", &reservoirListB);    
    ...
    if (reservoirList_aux != NULL) 
		reservoirTable_free(reservoirList_aux);
    reservoir_free(&rat);
    reservoir_free(&bat);
    reservoir_free(&monkey);
...
bool run_pr1_ex3(tTestSection* test_section) {
    ...
    infectiousAgent_free(&influenzaB);
    infectiousAgent_free(infectiousAgent_aux);
    reservoirTable_free(&reservoirListA1);
...
bool run_pr1_ex4(tTestSection* test_section) {
 bool passed = true, failed = false;
    ...
    tInfection COVID_19_China = {0};;
	tInfection COVID_19_SouthKorea= {0} ;
	tInfection COVID_19_Italy= {0};
	tInfection ebola_Sierra_Leone= {0};
    tInfection ebola_Liberia= {0} ;
	tInfection ebola_Guinea= {0};
	tInfection COVID_19_copy = {0};
    ...
    err = infection_init(&COVID_19_China, &COVID_19, "China", &dateOutbreak, 1, 0);
	...
	infection_update(&COVID_19_China, 80711, 3042);
    if(COVID_19_China.cases != 80712 ||  COVID_19_China.deaths !=3042) {        
        failed = true;
    }
	...
	err = infection_init(&COVID_19_SouthKorea, &COVID_19, "South Korea", &date, 1, 0);
    if(err) {        
        failed = true;
    }
	date.day = 31; date.month = 1; date.year = 2020;
	err = infection_init(&COVID_19_Italy, &COVID_19, "Italy", &date, 1, 0);
    if(err) {        
        failed = true;
    }  
    ...
    infectionAux = infectionTable_find(&infectionsTable, "SARS-CoV-2", "South Korea");
	...
	infection_init(&ebola_Sierra_Leone, &ebola, "Sierra Leone", &date, 10000, 3956);
	infection_init(&ebola_Liberia, &ebola, "Liberia", &date, 10000, 4809);
	infection_init(&ebola_Guinea, &ebola, "Guinea ", &date, 3800, 2500);
    ...
    if(infectionAux == NULL || !infection_equals(infectionAux, &COVID_19_China )) {    
        failed = true;
    }
	...
	if(mortalityRate != 0.0376886278f) {    
        failed = true;
    } 
