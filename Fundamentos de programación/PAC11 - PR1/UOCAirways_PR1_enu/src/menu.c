...
/*************** PR1 EX2  *****************/
    ...
    printf("Passenger name (max %d char, no spaces):\n>> ", MAX_NAME-1);
    scanf("%s",buffer);
    ...
    printf("Passenger surname (max %d char, no spaces):\n>> ", MAX_SURNAME-1);
    scanf("%s",buffer);
	length = strlen(buffer);
	if (length<MAX_SURNAME) {
		strncpy(passenger->surname,buffer,MAX_NAME);
	} else {
		*retVal = ERROR;
	}
	getchar();

	printf("Passenger document type (1 digit being 0=NIF 1=NIE 2=PASSPORT 3=DRIVING_LICENSE 4=OTHER DOCUMENTS):\n>> ");
	scanf("%d",&number);
	...
	printf("Passenger document number (max %d char, no spaces):\n>> ", MAX_DOC_NUMBER-1);
    scanf("%s",buffer);
	length = strlen(buffer);
	...
	printf("Passenger birthdate. Year:\n>> ");
	scanf("%d",&(passenger->birthDate.year));
	getchar();
	printf("Passenger birthdate. Month:\n>> ");
	scanf("%d",&(passenger->birthDate.month));
	getchar();
	printf("Passenger birthdate. Day:\n>> ");
	scanf("%d",&(passenger->birthDate.day));
	getchar();

	printf("Passenger country ISO code (2 digit char):\n>> ");
    scanf("%s",buffer);
	length = strlen(buffer);
	...
	printf("Does the passenger holds a customer card? (YES/NO):\n>> ");
	scanf("%s", buffer);
	...
	if (passenger->customerCardHolder == TRUE) {
		printf("Passenger customer card number (integer):\n>> ");
		scanf("%d",&(passenger->cardNumber));
		getchar();
		
		printf("Passenger fidelity points (integer):\n>> ");
